...
let ownerRepo: string = 'variantdev/go-actions';
let toolName: string = 'go-actions';
...
export async function getTool(version: string) {
  ...
  if (!toolPath) {
    ...
    toolPath = await acquireTool(version);
    core.debug(`${toolName} tool is cached under ` + toolPath);
  }
...
async function acquireTool(version: string): Promise<string> {
  //
...
function getFileName(version: string): string {
  ...
  const filename: string = util.format(
    'download/v%s/actions_%s_%s_%s.%s',
    version,
    version,
    platform,
    arch,
...
interface IReleaseRef {
  tag_name: string;
}

async function getAvailableVersions(): Promise<string[]> {
  let rest: restm.RestClient = new restm.RestClient(
    util.format('setup-%s', toolName)
  );
  let tags: IReleaseRef[] =
    (await rest.get<IReleaseRef[]>(
      util.format('https://api.github.com/repos/%s/releases', ownerRepo)
    )).result || [];
